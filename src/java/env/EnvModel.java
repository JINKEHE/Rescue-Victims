...
public class EnvModel extends GridWorldModel {
    ...
    // constructor with width, height, set of obstacles, set of possible
    // victims, whether there are wall
    ...
    public boolean isThatGridOccupied(int x, int y, String absoluteHeading) {
        if (absoluteHeading.equals("up"))
            return isOccpuied(x, y - 1);
        if (absoluteHeading.equals("down"))
            return isOccpuied(x, y + 1);
        if (absoluteHeading.equals("left"))
            return isOccpuied(x - 1, y);
        if (absoluteHeading.equals("right"))
            return isOccpuied(x + 1, y);
        return true;
    }
    ...
    public Location[] findOrderOfVictimsToVisit(Location currentLoc) {
        ...
        ArrayList<Location[]> permutations = getPermutations(0,
                victimsToVisit.toArray(new Location[victimsToVisit.size()]));
    ...
    // compute the cost for each possible order of victims to visit
    ...
    // Converts the array of locations to a string for printing.
