...
public class Playground extends Environment {
    ...
    // the path to find possible victims, which consists of relative directions to move
    ...
    // convert a plan consisting of locations the robot need to visit to a plan consisting of directions to move
	...
	// do a one step search - return the action that can reduce the most number of possible positions
	...
	// do a breadth first tree search - return the first action that can lead to a reduction in the number of possible positions
    ...
    // the number of different scenario after this action/the total number of scenarios after this action
    private double howThisActionCanDistinguish(HashSet<Position> pool, int action) {
	    ...
	    for (Position pos : pool) {
		    // clone the position
	    	// if we use the original one, we need to move it back, which I don't want to do
