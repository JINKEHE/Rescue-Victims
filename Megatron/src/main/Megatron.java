...
public class Megatron {
	...
	private float getOneDistance() {
		...
		// if the distance is INIFINITY, try to move back or rotate a little bit to avoid it
		int count = 1;
		...
		while (Float.isInfinite(distance)) {
			...
			if (count == 1) {
				...
				count = -1;
			// if moving back does not work, rotate one degree and scan again
			} else if (count == -1) {
				pilot.rotate(1);
				ultrasonicDistSP.fetchSample(ultrasonicDistSample, 0);
				distance = ultrasonicDistSample[0] * 100;
	...
	private double getAccurateDistance() {
		...
		while (true) {
			...
			// if each distance we got is close to the average, then this set of data is reliable
			// if any distance in the data set is very different from the average, then this set
	...
	// 1. keep moving forward until black tape is scanned and then stop
	// 2. move forward for a fixed length distance
	private void goForward() {
		pilot.forward();
		while(true) {
			if(getColorName().equals(BLACK)) {
				pilot.stop();
				pilot.travel(FIXED_LENGTH_DISTANCE);
				break;
			}
		}
	}
	...
	private String moveRelatively(String relativeDirection) {
